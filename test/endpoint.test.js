...
const fakePage = { handlers: {}, mouse: { async wheel() {} }, on(e, fn) { this.handlers[e] = fn; }, async goto() {}, async waitForTimeout() {}, async evaluate(fn) {
...
test('navegacao visual para rota de reposts usa DOM', async () => { scenario.tabFound = false; scenario.apiItems = []; scenario.domItems = [makeItem('20', 'Tecca', 'liltecca')]; const r = await post({ username: 'el_ty2', scrolls: 1 }); assert.strictEqual(r.json.items.length, 1); assert.ok(r.json.debug.strategies.some(s => s.startsWith('dom-visible'))); });
test('fallback visual le cartoes renderizados', async () => { scenario.tabFound = false; scenario.domItems = [makeItem('30', 'Tecca', 'liltecca')]; const r = await post({ username: 'el_ty2', scrolls: 1 }); assert.strictEqual(r.json.items.length, 1); assert.ok(r.json.debug.strategies.some(s => s.startsWith('dom-visible'))); });
